...
@Service
public class StaticPartService {

    private final ObjectMapper objectMapper;

    public StaticPartService(ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
    }

    public PlacementResult<Boolean> placeStaticPart(
            String contentType,
            String conveyorName,
            String label,
            byte[] body,
        ...
        StaticPartLoader<Object> loader = objectConveyor.staticPart();
        loader = loader.label(typedLabel);

        Map<String, String> remaining = new HashMap<>(requestParams);

        rejectUnsupportedParam("ttl", remaining);
        rejectUnsupportedParam("creationTime", remaining);
        rejectUnsupportedParam("expirationTime", remaining);
        rejectUnsupportedParam("id", remaining);
        ...
        boolean delete = parseAndRemoveDelete(remaining);
        ...
        appliedProps.put("delete", delete);
        ...
        if (delete) {
            ...
            if (body == null || body.length == 0) {
                throw new IllegalArgumentException("Static part value is required unless delete=true");
            }
            if (!StringUtils.hasText(contentType)) {
                throw new IllegalArgumentException("Content-Type is required when setting static part value");
            }
        ...
        return buildResult(future, requestTtl, label, appliedProps);
    }

    private void rejectUnsupportedParam(String key, Map<String, String> params) {
        if (params.containsKey(key)) {
            throw new IllegalArgumentException("Parameter '" + key + "' is not supported for static parts");
        }
    }

    private boolean parseAndRemoveDelete(Map<String, String> params) {
        if (!params.containsKey("delete")) {
            return false;
        }
        String raw = params.remove("delete");
        ...
        throw new IllegalArgumentException("Invalid boolean for delete: " + raw);
    }
    ...
    private Class<?> resolveTargetType(ConveyorMetaInfo<?, ?, ?> metaInfo, String contentType, Object typedLabel) {
        ...
        if (mt.isCompatibleWith(MediaType.APPLICATION_OCTET_STREAM)) {
            if (supported.contains(byte[].class)) {
                return byte[].class;
            }
        } else if (mt.isCompatibleWith(MediaType.TEXT_PLAIN)) {
            if (supported.contains(String.class)) {
                return String.class;
            }
