...
import com.aegisql.conveyor.persistence.jdbc.engine.oracle.OracleEngine;
...
public class OracleEngineTest {
	...
	private static final String SCHEMA = Tester.getOracleSchema();
	private static final String DATABASE = Tester.getOracleServiceName();
	private static final String PARTS = "PART_" + Tester.getTestClass().toUpperCase(Locale.ROOT);
	private static final String LOGS = "LOG_" + Tester.getTestClass().toUpperCase(Locale.ROOT);

	@BeforeAll
	public static void setUpBeforeClass() {
		BasicConfigurator.configure();
		assumeTrue(Tester.testOracleConnection());
		Tester.removeLocalOracleTables(PARTS, LOGS);
	}
	...
	public void testOracle() throws IOException {
		...
		cf.setUser(Tester.getOracleUser());
		...
		de.connectionFactory.setPort(Tester.getOraclePort());
