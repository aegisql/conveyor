...
public class OraclePerfTest {

	private static final String DATABASE = Tester.getOracleServiceName();
	private static final String ARCHIVE_PREFIX = "A_";

	JdbcPersistenceBuilder<Integer> persistenceBuilder = JdbcPersistenceBuilder.presetInitializer("oracle", Integer.class)
			.host(Tester.getOracleHost())
			.port(Tester.getOraclePort())
			.user(Tester.getOracleUser())
			.password(Tester.getOraclePassword())
			.autoInit(true)
...
//			.connectionFactory(ConnectionFactory.driverManagerFactoryInstance())
	...
	public static void setUpBeforeClass() {
		...
		assumeTrue(Tester.testOracleConnection());
		Tester.removeLocalOracleTables(
				"testParallelAsorted", "testParallelAsortedCompleted",
				"testParallelSorted", "testParallelSortedCompleted",
				"testParallelUnload", "testParallelUnloadCompleted",
				"testParallelAsorted1", "testParallelAsorted1Completed",
				"testParallelAsorted2", "testParallelAsorted2Completed",
				"testParallelSortedFile", "testParallelSortedFileCompleted",
				"testParallelSortedPersistence", "testParallelSortedPersistenceCompleted",
				"A_testParallelSortedPersistence", "A_testParallelSortedPersistenceCompleted"
		);
	}
	...
	public void setUp() {
		...
		System.out.println("--- OraclePerfTest " + new Date());
	}
	...
	Persistence<Integer> getPersitencePersistence(String table) {
		try {
			...
			Persistence<Integer> archive = persistenceBuilder
					.database(DATABASE)
					.partTable(ARCHIVE_PREFIX + table)
					.completedLogTable(ARCHIVE_PREFIX + table + "Completed")
					.labelConverter(TrioPart.class)
					.maxBatchSize(batchSize)
					.build();
